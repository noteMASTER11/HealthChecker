...
public sealed class SqliteMonitoringStore
{
            ...
            await using (var targetCommand = connection.CreateCommand())
            {
                ...
                while (await reader.ReadAsync(cancellationToken))
                {
                    targets.Add(new MonitoredTargetState
                    {
                        ...
                        LastCheckedUtc = reader.IsDBNull(5)
                            ? null
                            : DateTimeOffset.FromUnixTimeSeconds(reader.GetInt64(5)),
                        LastOnlineUtc = reader.IsDBNull(6)
                            ? null
    ...
    public async Task AppendProbeAsync(
        Guid targetId,
        ProbeResult result,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                // Target can be removed while a probe cycle is still finishing.
                // Ignore orphan probe writes in that short race window.
            }
        }
        finally
        {
        ...
        try
        {
            ...
            await using (var rangeCommand = connection.CreateCommand())
            {
                rangeCommand.CommandText = """
                                           ...
                                           """;
                rangeCommand.Parameters.AddWithValue("$targetId", targetId.ToString());
            ...
            bucketCommand.CommandText = """
                                        ...
                                        """;
            bucketCommand.Parameters.AddWithValue("$targetId", targetId.ToString());
    ...
    private async Task TryMigrateLegacyJsonAsync(SqliteConnection connection, CancellationToken cancellationToken)
    {
        ...
        {
            // Migration already succeeded. Backup failure should not break app startup.
        }
    }
